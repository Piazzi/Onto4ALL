 ...
 */
EditorUi.prototype.footerHeight = 0; //Footer Retirado
...
EditorUi.prototype.save = function (name) // FUNÇÃO CHAMADA DEPOIS DA saveFile
{
    ...
    if (name != null) {
        ...
        if (xml.length < MAX_REQUEST_SIZE) {
            ...
            mapForm.method = "POST";
            mapForm.action = "/saveXML";

            var mapXMLInput = document.createElement("input");
            mapXMLInput.type = "xml";
            mapXMLInput.name = "xml";
            mapXMLInput.value = xml;

            var token = document.createElement("input");
            token.setAttribute("_token", 'meta[name=csrf-token]');
            token.name = "_method";
            token.type = "hidden";
            token.value = "POST";

            var mapFileNameInput = document.createElement("input");
            mapFileNameInput.type = "text";
            mapFileNameInput.name = "fileName";
            mapFileNameInput.value = name;

            mapForm.appendChild(mapXMLInput);
            mapForm.appendChild(mapFileNameInput);
            mapForm.appendChild(token);
        ...
        try {
            if (Editor.useLocalStorage) {
                ...
                if (xml.length < MAX_REQUEST_SIZE) {
                    console.log(new mxXmlRequest(SAVE_URL, 'filename=' + encodeURIComponent(name) +
